$(document).ready(function () {
    ...
    $('#id_phone').on('change', function getClientData() {
        ...
        $.ajax({
            url: '/validate_client', data: {
                'phone': phone, 'csrfmiddlewaretoken': csrfToken,
            }, method: 'POST', mode: 'same-origin', success: function (response) {
                if (response.is_taken === true) {
                    ...
                    document.getElementById('count_orders').innerHTML = response.count_orders
                } else {
                    $('#id_name').val(null);
                    $('#id_address').val(null);
                    $('#id_home').val(null);
    ...
    // Запрет множественного клика отправки формы
    $(document).ready(function () {
    ...
    $(document).on('change', '#id_order_status', function setOrderStatus() {
        ...
        $.ajax({
            url: '/', data: {
                'order_status': order_status, 'order_id': order_id, 'csrfmiddlewaretoken': csrfToken,
    ...
    $(document).on('change', '#list_order_courier', function setOrderCourier() {
        ...
        $.ajax({
            url: '/', data: {
                'order_courier': order_courier, 'order_id': order_id, 'csrfmiddlewaretoken': csrfToken,
            }, method: 'POST', success: function () {
                $('#order_list_block')
                    .load(document.location.pathname + " #order_list_block > *")
            }
    ...
    $('#product-button').on('click', function addProductOnCart() {
        ...
        $.ajax({
            url: '/cart_add', data: {
                'product': product, 'csrfmiddlewaretoken': csrfToken,
            }, method: 'POST', // success: function () {
    ...
    $('.cart-item-qty').on('click', function () {
        ...
        $.ajax({
            method: 'GET', url: '{% url "change_item_qty" %}', data: data, success: function (data) {
    ...
    // Запрет отправки формы по кнопке "Enter"
